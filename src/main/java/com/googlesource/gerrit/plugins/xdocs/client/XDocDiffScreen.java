...
public abstract class XDocDiffScreen extends VerticalPanel {
  ...
  private void setRevisions(ChangeInfo change, String patchSetString) {
    ...
    if (i > 0) {
      this.base = parsePatchSet(patchSetString.substring(0, i));
      this.revisionA = getRevision(change, base);
      if (patchSetString.length() > i + 2) {
        this.patchSet = parsePatchSet(patchSetString.substring(i + 2));
        this.revisionB = getRevision(change, patchSet);
      } else {
        throw new IllegalArgumentException("Invalid patch set: " + patchSetString);
      }
    } else {
      this.patchSet = parsePatchSet(patchSetString);
      this.revisionB = getRevision(change, patchSet);
