...
public class XDocSideBySideDiffScreen extends XDocDiffScreen {
  static class Factory implements Screen.EntryPoint {
    ...
    public void onLoad(Screen screen) {
      ...
      screen.show(new XDocSideBySideDiffScreen(change, patchSet, file));
      screen.setWindowTitle(XDocScreen.getFileName(file));
    }
  }

  XDocSideBySideDiffScreen(String change, String patchSet, String file) {
    super(change, patchSet, file);
  }
  ...
  protected void display(ChangeInfo change) {
    ...
    Frame frameA =
        new Frame(XDocScreen.getUrl(change.project(), getRevisionSideA(), getFile()));
    frameA.getElement().setId(frameIdA);
    XDocScreen.resize(frameA, frameIdA);

    String frameIdB = "xdoc_sidebyside_diff_b_iframe";
    Frame frameB =
        new Frame(XDocScreen.getUrl(change.project(), getRevisionSideB(), getFile()));
